...
(() => {
  ...
  const CSS = `
    ...
    /* Slot 8 of the kit's home row. The button used to pick its own corner, and the
       reasoning is worth keeping for the PANEL, which still does: bottom-right is the
       game's own Comms dock (right: 20px, bottom: 0, 320×420), and this panel is ~500px
       tall, so it is anchored from the top — bottom-anchoring a panel that size pushes
       its head off a 720px screen. Drag either anywhere you like; both remember. */
    .pktw-fab { --pk-slot: 14; z-index: 2147482000; }
  ...
  const mount = () => {
    ...
    fab.addEventListener('dblclick', () => { ui.fab = null; writeJSON(K.ui, ui); fabDrag.reset(); });
